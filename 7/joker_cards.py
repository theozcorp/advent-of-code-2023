...
class Hand:
    ...
    def _calculate_hand_strength(self) -> int:
        ...
        joker_modifed_counts = sorted(label_counts.values(), reverse=True)
        if len(joker_modifed_counts) == 0:
            joker_modifed_counts.append(0)
        joker_modifed_counts[0] += self._joker_card_count
        return sum(1 << (count * 2) for count in joker_modifed_counts)
